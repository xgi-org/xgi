...
def _request_from_hypergraphx_data(
    ...
    rawdata = _download(url, cache=cache)
...
def _load_hypergraph(jsondata, nodetype=None, edgetype=None):
    ...
    for item in jsondata:
        ...
        if "hypergraph_type" in item:
            ...
            if edgetype is not None and edge_id is not None:
                edge_id = edgetype(edge_id)

            if edge_id is None:
                H.add_edge(interaction, **metadata)
            else:
                H.add_edge(interaction, id=edge_id, **metadata)
...
def _download(url, *, timeout=30, verify_ssl=False, cache=True):
    """Download raw data from a URL."""
    # TODO: Implement repository-specific caching if desired.
    #
    # Unlike the XGI-data repository, these files are gzipped JSON, so the
    # generic request_from_url_cached() utility cannot be used directly.
